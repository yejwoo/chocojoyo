  ...
  return (
    <main className="xs:border xs:border-disabled-200 bg-white max-w-[400px] max-h-[800px] absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 overflow-hidden">
      <div className="absolute w-full h-full">
        ...
        {/* 하트 */}
        <Image src={heart} className="absolute w-[240px] top-20 max-h-sm:top-10 left-1/2 -translate-x-1/2" alt="하트" />
        {/* 구름 */}
        <div className="absolute w-full h-96 max-h-sm:h-48 bottom-0 left-1/2 -translate-x-1/2">
          <Image src={cloud1} width={200} height={160} className="absolute bottom-32 max-h-sm:bottom-8 left-[-48px] animate-cloud-left-right" alt="구름" />
          <Image src={cloud2} width={160} height={160} className="absolute bottom-64 max-h-sm:bottom-40 right-[-48px] animate-cloud-left-right" alt="구름" />
        </div>
        {/* 초콜릿 오브젝트 */}
        <div className="absolute w-full h-56 max-h-sm:h-40 bottom-48 max-h-sm:bottom-36 left-1/2 -translate-x-1/2">
        ...
        {/* 슬로건 */}
        <p className="absolute top-64 max-h-sm:top-56 w-full text-center text-xl">내 손으로 꾸미는 달콤한 초콜릿을 만들어줘요</p>
        {/* 얀토 */}
        <Image src={yanto} width={200} height={200} className="absolute top-[45%] -translate-x-1/2 left-1/2 animate-bounce-up-middle" alt="얀토" />
        {/* 로고 */}
        <Image src={logo} className="absolute w-[360px] max-h-sm:w-[336px] top-20 max-h-sm:top-12 left-1/2 -translate-x-1/2" alt="초코조요" />
        {/* <p className="absolute bottom-24 w-full text-center text-gray-warm-200 text-xl">벌써 0명이 만들었어요!</p> */}
        {/* 버튼 */}
        <div className="absolute bottom-8 max-h-sm:bottom-4 left-1/2 -translate-x-1/2">
